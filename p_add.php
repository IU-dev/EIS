...
if (isset($_POST['submit'])) {
    foreach($_POST['fio'] as $key=>$item) {
        if ($_POST['fio'][$key] != "") {
            ...
            $data['i'] = "'" . $fio[1] . "'";
            $data['o'] = "'" . $fio[2] . "'";
            $data['username'] = "'" . $_POST['username'][$key] . "'";
            $data['group_id'] = "'" . $_POST['group_id'] . "'";
            $password = mt_rand(1000, 9999);
            $data['password'] = "'" . md5($password) . "'";
            $data['join_date'] = "'" . date("Y-m-d H:i:s", time()) . "'";
            $itog = $db->insert($data, 'users');
            echo $_POST['username'][$key] . '; ' . $_POST['fio'][$key] . '; ' . $password . '<br>';
        }
    }
    ...
    <form class="md-form border border-light p-5" action="p_add.php" method="post">
        ...
        <table>
            ...
            for($i = 1; $i <= 30; $i = $i + 1) {
                ...
                echo '<td><input type="text" id="textInput" name="username[]" class="form-control mb-4 fixed-15em" placeholder="ID системы ЕИС"></td>';
                echo '</tr>';
            }
        ?>
            </tbody>
